...
type ContentBySlugMap = Record<string, ContentBlock[]>;
...
function isContentBySlugMap(value: unknown): value is ContentBySlugMap {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
async function loadContentBySlugMap(): Promise<ContentBySlugMap> {
  const contentPath = path.join(process.cwd(), "data", "section-contentV2.json");
  const raw = await fs.readFile(contentPath, "utf8");
  const parsed: unknown = JSON.parse(raw);

  if (!isContentBySlugMap(parsed)) {
    return {};
  }
...
export default async function ContentPage({ params }: ContentPageProps) {
  ...
  const currentSubgroup = getExamSubgroupBySlugs(groupSlug, contentSlug);
  if (!currentSubgroup) {
    notFound();
  }
  ...
  const contentBySlugMap = await loadContentBySlugMap();
  const contentBlocks = contentBySlugMap[contentSlug] ?? [];
  const panelStyle: GroupColorStyle = { "--group-color": currentSubgroup.awsColor };

  return (
    <main className="section-shell">
      <Link href={`/v2/sections/${groupSlug}`} className="back-link">
        {"<- Back to group"}
      </Link>
        ...
        <nav className="section-nav" aria-label="Section navigation">
          <div>
            {previousSubgroupSlug && previousSubgroup ? (
              <Link
                ...
                href={`/v2/sections/${groupSlug}/content/${previousSubgroupSlug}`}
              >
          ...
          <div>
            {nextSubgroupSlug && nextSubgroup ? (
              <Link
                className="section-nav-link"
                href={`/v2/sections/${groupSlug}/content/${nextSubgroupSlug}`}
              >
